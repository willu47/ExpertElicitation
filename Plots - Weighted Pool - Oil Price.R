...
#   Fill array for density plots
    ...
    dists.m.w<-array(,c(N.expert.max,l.xaxis+1,l.params))
...
#############################################################################################
    ...
    pdf("Weighted Pool.pdf", width=6,height=6,onefile=TRUE)
...
### Calculate the linear average of the beliefs (WEIGHTED linear opinion pool)
	x<-1:25
	dists.m.w[x,,i] <- dists.m[x,,i] * weights[x,2]
	best.linear.pool.w<-apply(dists.m.w[x,,i],2,mean,na.rm=TRUE)


### Build the axes and plot the lines of the distributions    
    matplot(x.m[i,],t(dists.m[,,i]),xlim=c(minx.m[i],maxx.m[i]),
      ylim=c(0,max.dist.m[i]),type="l", lwd=2,lty=3,xlab="",ylab="p(x)",cex.axis=0.9,las=1,col="grey")
    axis(1,labels=NA,xlim=c(minx.m[i],maxx.m[i]))

### Add units to x-axis         
    title(main=labels[which(labels[,1]==dimnames(data.m)$Param[i]),2],
    xlab=labels[which(labels[,1]==dimnames(data.m)$Param[i]),3])

### Add the linear opinion pool to the plot    
    matlines(x.m[i,],best.linear.pool.m, lty=1, lwd=2,col="black")    

	matlines(x.m[i,],best.linear.pool.w, lty=1, lwd=2,col="red") 
...
drawline<-function(x,text,unit,rounding=0,colour="black",plot=0,pch=25){
    ...
    mar=c(0,0,0,0),cex=1.20)
}

    if(plot==0){
    points(x,0,col=colour,pch=pch,cex=1.20)
}
...
if(dimnames(data.m)$Param[i]=="Oil"){
  ...
  lines(density(hist_oil("density")),col="blue",lwd=2,lty=2)
    legend(legplace,,
      c(
        paste("Weighted Linear opinion pool"),
        paste("Un-weighted Linear opinion pool"),
        ...
        pch=c(NA,NA,24,25,NA),
        lty=c(1,1,NA,NA,2),
        lwd=c(2,2,NA,NA,2),
        col=c("red","black","black","red","blue"),
        bg="white",cex=legtextsz,bty="n")
}
