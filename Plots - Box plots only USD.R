...
#############################################################################################
    ...
    pdf("Box Plots.pdf", width=6,height=6,onefile=TRUE)
...
#############################################################################################
    ...
    boxplot(t(cumdists.m[,,i]*1.6),outline=F,horizontal=T,ylim=c(minx.m[i]*1.6,maxx.m[i]*1.6),
cex.axis=0.9,las=1,yaxt="n",range=0,lwd=1)

### Shows Expert Affiliation and Calibration rating on right axis      

    title(ylab="Expert",main=labels[which(labels[,1]==dimnames(data.m)$Param[i]),2],
    sub=labels[which(labels[,1]==dimnames(data.m)$Param[i]),5],
...
#############################################################################################
#
#   Add the historical ranges to the boxplot (depending on which parameter is described in boxplot)  
#  
#############################################################################################
...
strwidth<-60	# width of wrap
legtextsz<-0.8	# size of legend text (0<x<1)
legplace<-"bottomright"	#  location of legend
...
if (dimnames(data.m)$Param[i]=="Pop"){
  ...
  paste("UK population in 2010: ",signif(hist_pop("max"),digits=2)," million")),
    pch=c(24,25),lty=c(NA,NA),lwd=c(NA,NA),col=c("blue","red"),bg="white",cex=legtextsz,bty="n")
}

if(dimnames(data.m)$Param[i]=="GDP"){
  drawline(hist_gdp("mean")*100,"Historical GDP 1990 to 2010: "," %",2)
  legend(legplace,,
  c(paste("UK GDP growth 1990 to 2010: ",signif(hist_gdp("mean")*100,digits=2)," %",sep="")),
  pch=c(25),col="black",lty=c(NA),lwd=c(NA),
  ...
  drawline(hist_oil("max"),"Oil price in 2010: $"," /barrel",3,"red",0,25)
    legend(legplace,,
      c(
        ...
        pch=c(24,25),lty=c(NA,NA),lwd=c(NA,NA),col=c("black","red"),bg="white",cex=legtextsz,bty="n")
}

if(dimnames(data.m)$Param[i]=="calib"){
  drawline(301.2,,,4,"black")
  legend(legplace,,
  c("Actual length of Moscow underground: 301.2 km"),
    pch=c(25),lty=c(NA),lwd=c(NA),col="black",bg="white",cex=legtextsz,bty="n")
}

if(dimnames(data.m)$Param[i]=="Temp"){
  drawline(19.1,"","C",3,"black")
  legend("topleft",,c("Linear opinion pool",strwrap("Median standardised daytime living room temperature: 19.1ºC",width=strwidth)),
pch=c(NA,25),lty=c(1,NA),lwd=c(2,NA),col="black",bg="white",cex=legtextsz,bty="n")
}
if(dimnames(data.m)$Param[i]=="GHG"){
  drawline(14.8*1.6,"Average price of CO2 in the EU ETS in 2010: $","",3,"black")
  legend(legplace,,c(
  # "Linear opinion pool",
  "Average price of CO2 in the EU ETS in 2010: $23.7 /tCO2e"),
  pch=c(25),col="black",
    lty=c(NA),lwd=c(NA),bg="white",cex=legtextsz,bty="n")
}
